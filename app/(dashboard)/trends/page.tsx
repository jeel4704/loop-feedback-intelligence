...
export default function TrendsPage() {
  ...
  // Calculate dynamic theme growth heuristically
  ...
  const themeGrowth = (charts?.topThemes || []).map(theme => {
    ...
    return {
      name: theme.name,
      growth: "No historical trend data available yet",
      spike: isSpike,
      count: theme.count
    };
  });
            ...
            <div className="space-y-2.5">
              {themeGrowth.length === 0 ? (
                ...
                themeGrowth.map((theme) => (
                  <div
                    key={theme.name}
                    className="flex flex-col gap-2 rounded-xl bg-slate-50/70 dark:bg-dark-elevated/40 border border-slate-100 dark:border-dark-border px-4 py-3"
                  >
                    <div className="flex items-center justify-between">
                      <p className="font-bold text-slate-900 dark:text-gray-100 text-[13px]">{theme.name}</p>
                      {theme.spike ? (
                        ...
                        <Badge variant="slate" className="font-bold tracking-wide uppercase text-[9px] px-1.5 py-0.5 border dark:border-dark-border dark:text-dark-muted dark:bg-dark-card">Steady rise</Badge>
                      )}
                    </div>
                    <p className="text-[11px] font-semibold text-slate-400 dark:text-dark-muted italic">{theme.growth}</p>
                  </div>
                ))
              )}
            </div>
